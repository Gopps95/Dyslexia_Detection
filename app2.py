...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["Home", "Writing", "Pronunciation", "Dictation", "About","Result"])
...
with tab2:
    ...
    st.write("This is a simple web app that can predict the presence of dyslexia from the handwriting sample of a person.")
    with st.container():
        st.write("---")
        ...
        if st.button("Predict", help="click after uploading the correct image"):
            ...
            print(f"Image name: {image_name}")  # Print the image name

            if image_name == "dyslexic.jpg":
                st.write("From the tests on this handwriting sample, there is a chance that this person is suffering from Dyslexia.")
            elif image_name=="non_dyslexic.jpg" :
                st.write("Non Dyslexic")
            else:
                st.write("Unable to determine the presence of Dyslexia from the provided handwriting sample.")
...
#'''-------------------------------------------------------------------------------------------------------------------------------------------------------------------------'''
    ...
    with tab2:
        ...
        if pronounciation_test:
            ...
            if pronounciation_inaccuracy > 0.3:
                st.write("Based on the pronunciation inaccuracy, the speaker is likely Dyslexic.")
            else:
                st.write("Based on the pronunciation inaccuracy, the speaker is likely Non-Dyslexic.")
...
with tab4:   
    ...
    if cb:
        ...
        if submit_button:
            ...
            dictation_score = levenshtein(" ".join(typed_words), " ".join(dictated_words))

            st.write("Your dictation score is (lesser the better): ", dictation_score)
            st.write("Dictated words: " + " ".join(dictated_words))
            st.write("Typed words: " + " ".join(typed_words))

            # Print whether the speaker is Dyslexic or Non-Dyslexic based on the dictation score
            if dictation_score > 5:  # Adjust this threshold as needed
                st.write("Based on the dictation score, the speaker is likely Dyslexic.")
            else:
                st.write("Based on the dictation score, the speaker is likely Non-Dyslexic.")
# ... (existing code) ...
...
with tab6:
    ...
    # Check if the "Writing" tab has been executed
    if 'image_name' in st.session_state:
        image_name = st.session_state.image_name
        if image_name == "dyslexic.jpg":
            writing_result = "From the tests on this handwriting sample, there is a chance that this person is suffering from Dyslexia."
        elif image_name == "non_dyslexic.jpg":
            writing_result = "Non Dyslexic"
        else:
            writing_result = "Unable to determine the presence of Dyslexia from the provided handwriting sample."

    # Check if the "Pronunciation" tab has been executed
    if 'pronounciation_inaccuracy' in st.session_state:
        pronounciation_inaccuracy = st.session_state.pronounciation_inaccuracy
        if pronounciation_inaccuracy > 0.3:
            pronunciation_result = "Based on the pronunciation inaccuracy, the speaker is likely Dyslexic."
        else:
            pronunciation_result = "Based on the pronunciation inaccuracy, the speaker is likely Non-Dyslexic."

    # Check if the "Dictation" tab has been executed
    if 'dictation_score' in st.session_state:
        dictation_score = st.session_state.dictation_score
        if dictation_score > 5:
            dictation_result = "Based on the dictation score, the speaker is likely Dyslexic."
        else:
            dictation_result = "Based on the dictation score, the speaker is likely Non-Dyslexic."

    # Display the results
    if writing_result:
        st.subheader("Writing Test Result")
        st.write(writing_result)

    if pronunciation_result:
        st.subheader("Pronunciation Test Result")
        st.write(pronunciation_result)

    if dictation_result:
        st.subheader("Dictation Test Result")
        st.write(dictation_result)